...
std::string serverprinthand(std::vector<int>& hand)
{
    std::string handstr = "";
    for (int i = 0; i < hand.size(); i++)
    {
        if (hand[i] % 13 == 12)
            handstr += suit(hand[i])+ "Q ";
        
        else if (hand[i] % 13 == 11)
            handstr += suit(hand[i])+ "J ";

        else if (hand[i] % 13 == 0)
            handstr += suit(hand[i]) + "K ";
        
        else if (hand[i] % 13 == 1)
            handstr += suit(hand[i]) + "A ";

        else
            handstr += suit(hand[i]) + std::to_string(hand[i] % 13) + " ";        
    }
    return handstr;
}
...
void cardchanger(std::vector<int>& hand)
{
    ...
    sendstring(new_socket,  "changes " + std::to_string(num) + " cards.");
    if (num != 5)
    {
        for (int i = 0; i < num; ++i)
        {
...
void servercardchanger(int socket, std::vector<int>& hand)
{
    sendstring(socket, serverprinthand(hand));
    sendstring(socket, "CARDCHANGER");
    std::string luku = "";
    luku = recvstring(socket);
    int num = std::stoi(luku);
    std::cout << "Player2 changes " << num << " cards.\n";
    if (num != 5)
    {
        for (int i = 0; i < num; ++i)
        {
            std::string tmp = "";
            tmp = recvstring(socket);
